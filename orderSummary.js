...
for (i=0; i<tellimus.rows.length;i++) {
    index=i
    if (index>=0) {
    console.log(`Toote nimi: ${tellimus.rows[index].name} ; Toote kogus: ${tellimus.rows[index].amount} ; Toote koguhind: ${tellimus.rows[index].price * ( 1 + tellimus.rows[index].vat) * tellimus.rows[index].amount * 100 / 100}€`)
    kogusumma = kogusumma + tellimus.rows[index].price*1.2*tellimus.rows[index].amount*100/100
    }
}
